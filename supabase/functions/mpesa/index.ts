...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      ...
      // Security: Validate order details on server side to prevent price tampering
      const { data: order, error: orderError } = await supabase.from("orders").select("id, buyer_id, total_amount, status").eq("id", orderId).single()
      if (orderError || !order || order.buyer_id !== user.id || order.status !== "pending") {
        return new Response(JSON.stringify({ error: "Invalid or unauthorized order" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } })
      }
