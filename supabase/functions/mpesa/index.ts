...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      ...
      // 1. Authenticate user via JWT in Authorization header
      const authHeader = req.headers.get("Authorization")
      if (action !== "stkpush" || !orderId || !authHeader) {
        return new Response(JSON.stringify({ error: "Unauthorized or invalid request" }), {
          status: 401,
      ...
      // 2. Validate order ownership and retrieve untampered total_amount from DB
