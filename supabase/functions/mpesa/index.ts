...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      ...
      // Query order from database to validate amount, status, and ownership (prevent price manipulation)
      ...
      if (orderError || !order || order.status !== "pending" || order.buyer_id !== user.id) {
