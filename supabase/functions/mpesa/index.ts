...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      // Fetch order details from DB to prevent price manipulation and check ownership
      ...
      if (order.status !== "pending") {
        return new Response(JSON.stringify({ error: `Cannot pay for order in ${order.status} status` }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }
