...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      const { data: { user } } = auth ? await supabase.auth.getUser(auth) : { data: { user: null } }
      if (!user) return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: corsHeaders })

      const { action, phoneNumber, orderId } = await req.json()
      const { data: order } = await supabase.from("orders").select("total_amount, status, buyer_id").eq("id", orderId).single()
      if (!order || order.status !== "pending" || order.buyer_id !== user.id || action !== "stkpush") {
