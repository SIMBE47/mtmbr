...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      // Fetch order details from database using service role to verify ownership and amount
      ...
      if (order.buyer_id !== user.id) {
        return new Response(JSON.stringify({ error: "Unauthorized order" }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }

      if (order.status !== "pending" || action !== "stkpush") {
