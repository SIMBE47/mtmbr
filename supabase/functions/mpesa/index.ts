  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      ...
      if (action !== "stkpush" || !orderId) {
        return new Response(JSON.stringify({ error: "Invalid action or request" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }

      // Validate order ownership, status, and get authentic amount from DB
      ...
      const verifiedAmount = Math.ceil(order.total_amount)
