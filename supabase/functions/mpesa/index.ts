...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      ...
      if (action !== "stkpush") return new Response(JSON.stringify({ error: "Invalid action" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } })

      // Verify and validate order details securely from DB to prevent client price/data manipulation
      const { data: order, error: orderErr } = await supabase.from("orders").select("total_amount, buyer_id, status").eq("id", orderId).single()
      if (orderErr || !order || order.status !== "pending" || order.buyer_id !== user.id) {
        return new Response(JSON.stringify({ error: "Invalid order state" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } })
      }
