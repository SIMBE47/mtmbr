...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      if (action !== "stkpush" || !orderId) return new Response(JSON.stringify({ error: "Invalid action" }), { status: 400, headers: corsHeaders })
      ...
      if (orderErr || !order || order.buyer_id !== user.id || order.status !== "pending") {
        return new Response(JSON.stringify({ error: "Invalid order" }), { status: 400, headers: corsHeaders })
      }
