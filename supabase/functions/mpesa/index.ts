...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      // 🛡️ Sentinel: Verify JWT to prevent unauthorized payment triggers
      if (!authHeader) {
      ...
      // 🛡️ Sentinel: Fetch order from DB to prevent price manipulation and verify ownership
      ...
      if (order.status !== "pending") {
        return new Response(JSON.stringify({ error: "Order is already processed or cancelled" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }
      ...
      const res = await fetch(`${DARAJA_BASE}/mpesa/stkpush/v1/processrequest`, {
        ...
        body: JSON.stringify({
          ...
          Amount:            Math.ceil(order.total_amount), // Use amount from DB, not from request
          PartyA:            phoneNumber,
