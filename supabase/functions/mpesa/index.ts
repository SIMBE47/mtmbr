...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      if (action !== "stkpush" || !orderId) return new Response("Invalid request", { status: 400, headers: corsHeaders })
      ...
      if (orderErr || !order || order.buyer_id !== user.id || order.status !== 'pending') {
        return new Response("Order not found or invalid", { status: 404, headers: corsHeaders })
      }
