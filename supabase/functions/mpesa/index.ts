...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      // 🛡️ Sentinel: Check Authorization header explicitly to prevent runtime errors
      ...
      // 🛡️ Sentinel: Robust JWT verification (case-insensitive and trimmed)
      ...
      if (action !== "stkpush" || !orderId) {
        return new Response(JSON.stringify({ error: "Invalid request action or parameters" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }

      // 🛡️ Sentinel: Retrieve authentic order total_amount and buyer_id from database
      ...
      // 🛡️ Sentinel: Validate that the authenticated user owns the order
      if (order.buyer_id !== user.id) {
        return new Response(JSON.stringify({ error: "Forbidden: Order ownership verification failed" }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }

      // 🛡️ Sentinel: Validate order is still in 'pending' status
      if (order.status !== "pending") {
        return new Response(JSON.stringify({ error: `Order cannot be processed because status is ${order.status}` }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        })
      }
