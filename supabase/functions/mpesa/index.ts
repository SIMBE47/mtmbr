...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      // Security: Explicitly check Authorization header first to prevent TypeErrors
      ...
      // Security: Extract and trim token case-insensitively
      ...
      // Security: Fetch authoritative order details from database using Service Role to prevent price manipulation
      ...
      // Security: Validate order ownership (buyer_id check)
      ...
      // Security: Validate order status is pending
