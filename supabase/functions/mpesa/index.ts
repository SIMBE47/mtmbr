...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      if (action !== "stkpush" || !orderId) return new Response(JSON.stringify({ error: "Invalid request" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } })

      // Authoritative check: fetch order directly from DB to prevent price manipulation
      ...
      if (orderError || !order || order.buyer_id !== user.id || order.status !== 'pending') {
        return new Response(JSON.stringify({ error: "Invalid order or unauthorized" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } })
      }
