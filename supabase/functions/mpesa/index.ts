...
serve(async (req) => {
  ...
  if (req.method === "POST" && (url.pathname.endsWith("/mpesa") || url.pathname.endsWith("/mpesa/"))) {
    try {
      ...
      // SECURITY: Fetch authoritative amount and verify ownership/status from DB
      ...
      if (orderError || !order || order.buyer_id !== user.id || order.status !== 'pending') {
        return new Response('Order not found or unauthorized', { status: 403, headers: corsHeaders })
      }
