...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      if (action !== "stkpush" || !order || order.buyer_id !== user.id || order.status !== "pending" || Math.ceil(order.total_amount) !== Math.ceil(amount)) {
        return new Response(JSON.stringify({ error: "Invalid request or order" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } })
      }
