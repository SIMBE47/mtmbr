...
serve(async (req) => {
  ...
  if (req.method === "POST" && url.pathname.endsWith("/mpesa")) {
    try {
      ...
      if (!user || !order || order.buyer_id !== user.id || action !== "stkpush") {
        return new Response(JSON.stringify({ error: "Unauthorized or invalid request" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } })
      }
      ...
      const res = await fetch(`${DARAJA_BASE}/mpesa/stkpush/v1/processrequest`, {
        ...
        body: JSON.stringify({
          ...
          TransactionType: "CustomerPayBillOnline", Amount: Math.ceil(order.total_amount),
          PartyA: phoneNumber, PartyB: DARAJA_SHORTCODE, PhoneNumber: phoneNumber,
          CallBackURL: DARAJA_CALLBACK_URL, AccountReference: "THRIFTR", TransactionDesc: `Order ${orderId}`,
        }),
      })
